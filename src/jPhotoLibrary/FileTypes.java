...
public enum FileTypes {
	...
	EXCLUDED {		
		@Override
			public String toString() {
				return systemFilesTypesUnEsccaped() + "," + settingsFilesTypesUnEscaped();
			}	
	};
	
	public String forRegex() {
		return toStrForRegex(this.toString());
	};
	
	private static String toStrForRegex(String nonEscapedStr) {
		//file types contents symbols, that we need to escape for regex
		String escaped = nonEscapedStr.replaceAll("([\\\\+*?\\[\\](){}|.^$])", "\\\\$1");
		//replace "," with "|" for regex (jpg|png) construction
		String barDivided = escaped.replace(",", "|");
		return barDivided;		
	};
	
	private static String systemFilesTypesUnEsccaped() {
		return "DLL,ICO,LNK,DRV,SYS,CAB,CUR,CPL,DMP,ICNS,DESKTHEMEPACK,HLP,SWP,MSP"
	...
	private static String settingsFilesTypesUnEscaped() {
		return "INI,CFG,PRF,ZAP,INF,MST,PLIST,RDF`,PRX,CSF,PSF,ICC,RDP,PROPERTIES,ICA"	
